/**
 * Vendored subset of `zoo-code/src/integrations/misc/extract-text.ts`.
 *
 * Only the three pure text-formatting helpers used by the vendored
 * MultiSearchReplaceDiffStrategy are carried here (verbatim from upstream):
 *
 *   - addLineNumbers
 *   - everyLineHasLineNumbers
 *   - stripLineNumbers
 *
 * The full upstream file also contains PDF/DOCX/XLSX extractors, binary-file
 * detection and indentation slicing, none of which the headless harness needs —
 * they pull in `pdf-parse`, `mammoth`, `isbinaryfile` and other GUI/service
 * dependencies that are not worth carrying.
 */
