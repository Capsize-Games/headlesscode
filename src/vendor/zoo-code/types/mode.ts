...
export const DEFAULT_MODES: readonly ModeConfig[] = [
	{
		...
		// Deviates from upstream Zoo/Roo Code (which gives architect no
		// "command" group at all, so it can't run anything — not even
		// read-only exploration commands like `grep`/`find`/test runners).
		// Headless architect sessions need real exploration, so this adds
		// "command" and drops the upstream .md-only "edit" group entirely:
		// architect gets every read/search tool plus a shell, but zero
		// write capability (see TOOL_GROUPS in shared/tools.ts — "edit" is
		// the only group backed by write_to_file/apply_diff).
